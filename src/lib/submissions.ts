...
export const recordSubmission = createServerFn({ method: "POST" })
  .validator((data: { result: Result; answers: Answers }) => data)
  .handler(async ({ data }) => {
    try {
      ...
      await sql`
        insert into test_submissions (
          id, ip, user_agent, user_id, triad_code, primary_subtype, confidence, result, answers
        ) values (
          ${id},
          ${clientIp()},
          ${clientUa()},
...
export const listSubmissions = createServerFn({ method: "GET" }).handler(
  async () => {
    ...
    const rows = await sql<{
      ...
      select id, created_at, ip, user_id, triad_code, primary_subtype, confidence, result
